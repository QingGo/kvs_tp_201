...
fn get_root_logger() -> slog::Logger {
    ...
    let root_logger = slog::Logger::root(drain, o!("process" => "redis_ping_server"));
    root_logger
}
    ...
    loop {
        ...
        let _: JoinHandle<Result<()>> = std::thread::spawn(move || {
            ...
            loop {
                ...
                let response = handler(&buf[..n]).unwrap_or(RESP::Error(
                    format!("ERR unknown command: {}", request_string).to_string(),
                ));
